...
def predict():
    try:
        ...
        x_master = [{key: data[key] for key in expected_features if key in data}]
        ...
        if master_pred == 0:
            ...
            end_time = time.time()
            log_entry = [time.ctime(), data, master_pred, "N/A", "N/A", "N/A", "No Action", end_time - start_time]
        ...
        for attack_type, slave_url in SLAVE_NODES.items():
            try:
                response = requests.post(slave_url, json={"data": data})
                if response.status_code == 200:
                    ...
                    log_entry = [
                        time.ctime(), data, master_pred, attack_type, slave_pred, 
                ...
                log_entry = [
                    time.ctime(), data, master_pred, attack_type, "-1", "N/A", "Error", time.time() - start_time
                ]
                with open(LOG_FILE, mode='a', newline='') as log_file:
                    csv.writer(log_file).writerow(log_entry)
